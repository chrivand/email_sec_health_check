...
# run script like this: >>> python3.6 email_health_check_new.py esa_config.xml
    ...
    def dmarc_check(self,domainname,approved_nameservers,approved_mx_hostnames):
        ...
        if ns["warnings"]:
            ...
            for warning in ns_warnings:
                self.add_remark_warning(warning)
        ...
        for host in hosts:
            if host["tls"] == False:
                ...
                self.add_remark_warning(tls_compliance)
        ...
        if "error" in dmarc:
            ...
            self.add_remark_warning(dmarc_compliance)
        else:
            dmarc_compliance = [
                { 'domain':domainname, 'dmarc_status':'compliant', 'error':None }
            ]
            self.add_remark_ok(dmarc_compliance)           
...
if __name__ == "__main__":
    ...
    approved_nameservers = ["<name-server1>","<name-server2>"]
    approved_mx_hostnames = ["<host-name-server1>","<host-name-server2>","<host-name-server3>"]
    ehc.dmarc_check(domainname,approved_nameservers,approved_mx_hostnames)
